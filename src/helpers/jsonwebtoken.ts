...
export const createAccessToken = (_id: Types.ObjectId): string => {
  return JWTOKEN.sign({ _id, type: "access" }, SECRET_KEY_JWT, { expiresIn: ACCESS_TOKEN_EXP });
};

export const createRefreshToken = (_id: Types.ObjectId): string => {
  return JWTOKEN.sign({ _id, type: "refresh" }, SECRET_KEY_JWT, { expiresIn: REFRESH_TOKEN_EXP });
};
