...
export const GetPantController = async (req: Request, res: Response) => {
  const clotheModel = req.dbClient.model('clothes', ClothesSchema)

  try {
    const pants = await clotheModel.find({ product: "pant" })
    return res.status(200).json({ success: true, message: "Successfully obtained pants.", pants: productos.pantalones })
  } catch (error) {
    console.error(error)
...
export const PostPantController = async (req: Request, res: Response) => {
  const clotheModel = req.dbClient.model('clothes', ClothesSchema)
